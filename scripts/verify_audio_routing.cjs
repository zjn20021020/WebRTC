...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('input[name="audioMode"][value="headphones"]').isChecked(), true);
    assert.equal(await page.locator('#audioModeStatus').textContent(), '自动识别');
    ...
    await page.locator('input[name="audioMode"][value="speakers"]').check();
    await wait('speakers', true);
    assert.equal(await page.evaluate(() => activeConnection.audioRoute.source), 'manual');
    await change('Headphones (USB)', 'headphones');
    await page.waitForTimeout(2200);
    await wait('speakers', true);
    assert.equal(await page.locator('#audioModeStatus').textContent(), '手动设置');
    results.manual_override_survives_detection = true;
    await disconnect();
    await page.locator('#connect').click();
    await wait('speakers', true);
    await page.waitForFunction(() => activeConnection?.peerConnection?.connectionState === 'connected');
    assert.equal(await page.evaluate(() => activeConnection.audioRoute.source), 'manual');
    results.manual_override_survives_reconnect = true;
    await page.locator('#audioModeAuto').click();
    await wait('headphones', false);
    assert.equal(await page.locator('input[name="audioMode"][value="headphones"]').isChecked(), true);
    results.restore_automatic = true;
    ...
    assert.equal(await page.locator('#audioModeStatus').textContent(), '设置未生效');
    ...
    assert.equal(await page.locator('input[name="audioMode"]').count(), 2);
    await page.locator('input[name="audioMode"][value="headphones"]').check();
    await wait('headphones', false);
    assert.equal(await page.locator('#audioModeStatus').textContent(), '手动设置');
    results.manual_headphone_fallback = true;
    await disconnect();
