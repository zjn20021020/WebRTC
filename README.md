...
当前没有语义意图分类或说话对象判断。AI 回答期间，另一句文本满足上述条件就视为插话，包括对旁人说的普通话语；不要求必须说“停”。这些规则用于过滤语气词和不稳定识别，不能可靠区分用户有意打断与无关讲话。
...
上述历史云联调使用 20% 增益。随后按听感反馈将疑似插话增益调整为 50%，并通过对应音频增益及打断单元测试；原始证据保留当时的数值。
