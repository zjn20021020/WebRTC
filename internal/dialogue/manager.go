...
func New(model LanguageModel, speech SpeechSynthesizer, emit func(Event)) *Manager {
	return &Manager{model: model, speech: speech, emit: emit, seen: make(map[string]bool)}
}
...
func (m *Manager) Accept(event asr.Event) {
	if event.UtteranceID == "" || strings.TrimSpace(event.Text) == "" {
		return
	}
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	t := &turn{epoch: m.epoch, utteranceID: event.UtteranceID, ctx: ctx, fail: cancel,
		cancel: func() { cancel(context.Canceled); timeoutCancel() }, frames: make(chan []byte, 250)}
	m.current = t
