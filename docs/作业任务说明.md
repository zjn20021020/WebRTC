...
待播队列最多 250 帧，约 5 秒。RTP 时钟每 20ms 取出一帧；没有音频时发送静音保持媒体链路。duck 在服务端把 PCM 样本乘以 0.2 后重新编码，音频队列继续消费，不调用浏览器 `pause()`。
...
后两次采样都早于 05:17:56.797 的恢复事件；其间上下行包数和下行能量均增加，说明播放期间上行仍活着，duck 期间下行保留了声音。服务端增益是否为 20% 则由 PCM 样本 RMS 比例单元测试验证；不同语音片段的累计能量不直接用来推算增益。
...
状态变为 `ducking`，服务端音量降到 20%，约 -14dB。旧轮 LLM/TTS 保持工作，队列继续按序播放，RTP 不改成整段静音。如果没有得到确认，VAD 检测到约 500ms 静音后再等待 800ms，然后恢复原音量；连续噪声最多保持 duck 4 秒。重复开始事件不无限延长窗口。
