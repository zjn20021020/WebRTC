...
[迪莫 SKILL.md](../internal/home/skills/dimo/SKILL.md) 以 `go:embed` 加载，固定伙伴身份、中文短句语气和能力边界。六类动作分类使用 DeepSeek 原生 `tools` / `tool_choice:required`，服务端验证单一白名单 function call、空参数对象和完整结束标记；额外正文、拒绝、未知工具、多个调用或无效参数不执行。动作分类失败时用固定语音询问澄清，`action_result` 和服务端日志标记 fallback 与原因。
