...
按用户确认的“先用接近迪莫感觉的童声”及后续“声音太沉、不够清亮”的听感反馈，当前默认使用腾讯 `101016`（智甜，精品女童声），替换首版的 `603002`（软萌心心）。[官方音色列表](https://cloud.tencent.com/document/product/1073/92668)未提供名为迪莫的公开音色，因此此处是角色近似配音，未做原声复刻。身份和台词由角色 skill / 工具决定，实际发声由 `TENCENT_TTS_VOICE_TYPE` 指定，角色 prompt 本身不会改变音色。
...
首版音色 `603002` 的[真实 WebRTC 回归记录](evidence/child-voice.json)验证了种菜期间问题缓存、施肥指令确认打断并丢弃旧问题、施肥十次结束后回答新问题。浏览器接收 1476 个 RTP 包，累计音频能量 2.2230；虚拟麦克风及物理输出静音的验证边界同下文。`go test ./...` 和 `go vet ./...` 均通过。

后续将配置改为 `101016`，实际流式试听收到 132 个分片，首包 430ms，音频 7798ms，RMS 3104，正常收到 final；备选 `502007`（智小虎）也成功合成。此轮只更改音色 ID，首版完整 WebRTC 回归记录不标成智甜音色的实测结果。
...
固定验收集见 [验收测试说明](验收测试说明.md)与 [cases.json](../testdata/acceptance/cases.json)，`home-acceptance-v1` 共 60 例，包含 18 个动作和 42 个打断分类；按稳定 ID 固定输入、历史、partial/final 与预期。默认重复 3 次，经生产客户端真实调用 DeepSeek 并执行严格协议校验。另有五个媒体场景固定验证延后安排、首次夸赞、明确替换、直接切换与打断清旧缓存，输入经实际 WebRTC/ASR/LLM/TTS。

`node scripts/verify_acceptance.cjs` 一键构建独立测试服务、准备缺失音频、运行测试并生成 Markdown/JSON 报告。结果保存在带时间戳的新目录，失败保留原始事件与截图，未执行不记为通过；详情与指标分母见验收说明。
...
自动统计严格通过率、请求错误率、动作混淆矩阵、有效决策的误/漏打断率、按类别及样例的结果、重复波动和 nearest-rank P50/P95。异常 false 不算分类成功，超时纳入请求延迟；媒体指标按轮次去重，直接、打断后和缓存派发分开统计。媒体记录的旧轮恢复是事件层检查，物理尾音尚未测量。
...
2026-09-12 固定验收完整基线见 [自动生成报告](evidence/acceptance-20260912/report.md)：60 例每例 3 次，动作 54/54、打断 126/126，误打断 0/60、漏打断 0/66，分类请求错误 0。动作请求 P50/P95 为 906/1216ms，打断请求为 802/919ms。五个真实 WebRTC 场景全部通过，旧轮恢复、动作/打断兜底与动作重试均为 0。

直接响应 final 到首音频发送为 P50/P95 1608/1788ms（5 轮），确认打断后的新轮为 2520/2555ms（3 轮）。缓存派发单独统计；浏览器确认事件到取消事件为 130/140ms（3 次），不等于实际开口到耳机静音。当前使用 `101016` 音色、PCMU/8kHz。JSON 保存每例结果、混淆矩阵、样本数、时间戳和素材摘要；这批固定输入的重复结果不当作线上准确率或 SLA。
...
需要 Go 1.22 或更高版本、支持 WebRTC 的浏览器。在根目录 `.env` 配置腾讯 AppID、SecretId、SecretKey，以及 DeepSeek API Key 和 URL，字段见 `.env.example`。默认模型 `deepseek-v4-pro`，腾讯童声音色 `101016`（智甜）。
