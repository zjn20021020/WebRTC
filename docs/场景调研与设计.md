...
[SKILL.md](../internal/home/skills/dimo/SKILL.md) 包含 YAML 元信息、身份、语气、事实边界和对话规则。Go 使用 `go:embed` 编入程序，在动作分类和通用问答的 system 消息中加载；这是一份应用运行时角色 skill，不是安装到开发助手全局的工作技能。修改后需重新编译/启动服务。
...
flowchart TD
  ...
  Action -->|异常| Clarify[固定语音澄清 不执行动作]
...
[action.md](../internal/llm/prompts/action.md) 配合六个 OpenAI 兼容 function definitions，调用相同的 `deepseek-v4-pro` 非思考模型。请求使用 `tools`、`tool_choice:"required"`、温度 0、128 token 上限和 5 秒请求期限。输入包含最新完整 ASR 和有限历史；分类只作用于最新请求，不能重放历史任务。
...
分类失败走固定澄清语音“小洛克，我还没确认你想做什么，请再说一遍吧。”，不执行任何工具、不自动重试。日志如 `action epoch=2 fallback=true route=clarification reason=invalid_output latency_ms=...`。动作分类失败与打断判定 false 的含义不同，不能混用。
...
`go test ./...` 和 `go vet ./...` 验证编译、协议与轮次集成。关键测试包括：工具白名单和原生协议拒绝、分类请求取消、分类异常固定澄清、浇水执行器取消及迟到输出拒绝、施肥十次保持播放、夸赞 false 缓存、施肥发送完成后才开始贴贴十次。
