...
- 用户开始说话时疑似插话先 duck 到 50%。直接命令执行另一项农务默认判 true，不需要先停止旧任务的措辞；明确延后、重复当前动作、夸赞和鼓励判 false。
- 普通问题转入 `general_qa` 需要明确停止、暂停或优先回答才打断；仅换一个话题、询问方法/状态不打断。直接从问答或贴贴切到农务也判 true。
- “等一下再去种地”表示等当前任务结束再种菜，判 false 并缓存；“等一下”单独成为 final 时才是暂停请求。“别浇水了，等一下再去种地”明确取消当前任务，仍判 true。识别中的逗号不能单独当作暂停依据。
- true 实际执行打断时，取消当前工具/问答轮次、LLM/TTS 请求和待播音频，同时丢弃其他待处理文本/草稿，只保留触发打断的新指令，在新 turn 等 final 并分类。false 仅缓存 final，当前轮播完后 FIFO 执行。
...
flowchart TD
  ...
  Intent -->|true| Cancel[取消旧工具/LLM/TTS 清音频与旧输入缓存 新 turn]
...
2026-09-12 按用户补充要求调整优先级：明确停止/重启/替换优先；否则明确延后优先于动作切换；否则新的、不同的农务动作默认立即替换。四种农务为 `water/plant/harvest/fertilize`，种菜/种地/种田是同一动作，重复同一动作不打断。`general_qa`、夸赞与亲密请求不隐含立即切换，其中问答需要明确暂停或优先回答；“先回答我...”属于明确替换，无须匹配固定停止口令。语义仍由 LLM 判定，未添加根据农务关键词直接取消的旁路。

| 当前活动 | 用户输入 | interrupt | 处理 |
| --- | --- | --- | --- |
| 种菜 | 去施肥 | true | 取消种菜，在新轮执行施肥 |
| 施肥 | 去种菜 | true | 取消施肥，在新轮执行种菜 |
| 种菜 | 等种完再去施肥 | false | 种菜结束后处理 |
| 种菜 | 去种地 | false | 同类动作，不停止当前轮，final 按缓存规则处理 |
| 种菜 | 怎么施肥 / 一加一等于几 | false | 问答延后 |
| 种菜 | 先别种菜了，一加一等于几 | true | 取消种菜，转入问答 |
| 种菜 | 干得不错 / 贴贴 | false | 亲密互动延后 |

完整且稳定的“去施肥”partial 可提前获 true；“去施”仍不足以确认。“立即”不意味着零延迟，而是不等待旧动作完成；保持最长 2 秒模型请求、50% duck、最小 120ms duck、取消旧上下文/待播以及新轮等待 final 的完整闭环。
...
用户补充要求确认打断也放弃之前的待处理任务。`confirmLocked` 先分离触发句，再调用 `clearInterjectionsLocked` 清空其他完整句与草稿、取消意图请求并记住被丢弃的句子 ID，然后停止旧轮并建立新轮。触发句不能一起标记为已处理，否则 partial 打断后同句 final 会被误丢弃。以上在同一锁内完成，尚未执行的 true 被撤回不会清缓存。`input_queue` 广播零计数，日志用 `inputs_dropped` 记录清理数量；之后的新输入继续按通常的 true/false 规则处理。这里清理的是待处理输入，已完成的对话历史仍供上下文使用。
...
[home-switch-classifier.json](evidence/home-switch-classifier.json) 保存 6 个动作分类及 33 个打断样例：四种农务的 12 种交叉切换、4 个同类请求、延后语义、普通/明确问答、夸赞、引用、否定和 partial 完整度均符合预期。样例包括从 `general_qa/affection` 直接转入农务。单元测试同时覆盖明确停止与直接“去施肥”的完整工具取消，拒绝旧工具迟到输出，并验证后续缓存仍等新工具播放结束。小样本通过不作为模型准确率承诺。

```powershell
E:\go\bin\go.exe run ./cmd/verify-home -suite switch
E:\go\bin\go.exe run ./cmd/demo-fixtures -scene home-switch
node scripts/verify_home.cjs --switch
```

真实语音 [home-switch-voice.json](evidence/home-switch-voice.json) 验证“去种菜 → 去施肥 → 一加一等于几”：UTC 07:58:20.672 ASR 收到没有停止措辞的“去施肥。”，07:58:21.499 判 true（824ms），07:58:21.632 取消种菜并清除 250 帧，TTS 当时仍活动；07:58:22.530 开始施肥。普通问题在 07:58:25.808 判 false 并缓存，施肥十次于 07:58:38.888 完成后才转入问答。回归用腾讯合成的虚拟麦克风语音、物理输出静音，保留真实云端与 WebRTC 链路；明确停止后转问答另外由上述文本对照覆盖。

### 确认打断清空待处理输入

[home-clear-buffer-voice.json](evidence/home-clear-buffer-voice.json) 验证先在种菜时缓存问题，再说“去施肥”打断：UTC 08:10:31.109 旧问题判 false 入队，08:10:33.752 取消种菜前已广播输入队列为零，服务端日志 `inputs_dropped=1`。旧问题的句子 ID 没有后续 `input_dispatched`。施肥过程中重新提出的问题在 08:10:37.599 入队，施肥完成后于 08:10:51.689 开始回答；只产生这一次问答轮次。输入是腾讯合成的虚拟麦克风语音，物理输出静音；草稿清理、迟到判定/final 和撤回确认由单元测试覆盖。

复现命令：`go run ./cmd/demo-fixtures -scene home-switch`，随后 `node scripts/verify_home.cjs --clear-buffer`。
