# 跨 ASR final 合并与复合任务规划

本说明记录最初在 `dev` 开发、现纳入 `master` 交付的方案，保留原文件名以维持证据链接。当前包括短窗口输入合并、派发前缓存补充合并、最多六步顺序规划和完整打断清理；麦克风固定使用原耳机配置，回声处理改造暂缓。

正式方案见[实验报告 D.7/D.8](作业任务说明.md#d7-跨-asr-final-合并)，图示见[架构与状态机说明](架构与状态机说明.md)，测试方法与证据见[测试情况与证据链分析](测试情况与证据链分析.md)。以下按开发阶段保留结果；73 例、83 例和 98 例对应不同快照，不能混作当前全量重复验收。
...
规划初版的固定云验收为 `home-acceptance-v2-plan`：73 例，其中 29 个动作/规划用例、44 个打断用例；媒体集 `home-media-v2-plan` 为八场景。计划按有序数组严格比较，顺序错、少步骤、多步骤都失败。包含“去胶水”在农务命令中按“去浇水”理解的回归，同时“胶水怎么做”保持问答；这是 LLM 的上下文规则，未全局替换原始字幕。当前固定集为 `home-acceptance-v4-continuation`，共 98 例（32 规划、52 打断、14 补充关系），媒体定义共 12 场景，后续增量及实际执行结果见第 7、8 节。
...
- 当前按用户要求移除耳机/外放选择，所有连接统一使用原耳机配置，三项处理均关闭，重连保持相同配置。回声处理改造暂缓，外放回声问题不计为已完成。
...
`go test ./...`、`go vet ./...`、五种宽度的 UI 回归通过。[采集专项](evidence/tts-capture-20260914/capture-stress.json)记录的是移除选项前的测试：通过 1800 个 dev 页面事件、显式 GC、AudioContext 暂停恢复，并验证当时的耳机/外放音轨设置。当前 `verify_capture.cjs` 改为验证固定耳机配置及重连后三项仍为 false，并检查选择控件已移除。该自动测试证明采集与设置逻辑，不代替物理耳机双讲测试；现场两条播放期 ASR 结果单独见上述片段。
